...
export const orderService = {
  ...
  try {
    ...
    if (!data) {
      console.warn('⚠️ Order not found:', orderId, 'for user:', userId)
      
      // Try to fetch without user filter to see if order exists
      const { data: anyOrder, error: _anyError } = await supabase
        .from('orders')
        .select('*')
        .eq('id', orderId)
        .maybeSingle()
      ...
      if (anyOrder) {
        console.warn('⚠️ Order exists but belongs to different user:', anyOrder.user_id)
        return {
          data: null,
